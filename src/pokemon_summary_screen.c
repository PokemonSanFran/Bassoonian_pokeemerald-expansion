...
#define PSS_DATA_WINDOW_INFO_POKEDEX_NO 0
#define PSS_DATA_WINDOW_INFO_SPECIES 1
#define PSS_DATA_WINDOW_INFO_ORIGINAL_TRAINER 2
#define PSS_DATA_WINDOW_INFO_ID 3
#define PSS_DATA_WINDOW_INFO_ABILITY 4
#define PSS_DATA_WINDOW_INFO_MEMO 5
...
#define MOVE_SELECTOR_SPRITES_COUNT 1
...
enum
{
    ...
    SPRITE_ARR_ID_EXTRATYPES,
    SPRITE_ARR_ID_SMALLICON = SPRITE_ARR_ID_EXTRATYPES + 2,
...
static void CreateMoveExtraTypeIcons(void);
static void SetMonTypeIcons(void);
static void SetMonTypeIconsExtra(void);
...
{
    gSummaryScreenPowAcc_Tilemap, 0, 12, 18, 0, 34
};
...
{
    [PSS_DATA_WINDOW_INFO_POKEDEX_NO] = {
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 2,
        .width = 17,
    ...
    [PSS_DATA_WINDOW_INFO_ID] = {
        ...
        .height = 2,
        .paletteNum = 6,
        .baseBlock = 461+(17*2*3),
    },
    [PSS_DATA_WINDOW_INFO_ABILITY] = { //Friendship
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 17,
        .width = 7,
    ...
    [PSS_DATA_WINDOW_INFO_MEMO] = { //Experience
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 12,
        .width = 17,
...
static u8 ShowSplitIcon(u32 split)
{
    if (sMonSummaryScreen->splitIconSpriteId == 0xFF)
        sMonSummaryScreen->splitIconSpriteId = CreateSprite(&sSpriteTemplate_SplitIcons, 48, 49, 0);
    ...
    //Conveniently hijack for other sprite displays
    SetMonTypeIconsExtra();
...
static void DestroySplitIcon(void)
{
    ...
    //Conveniently hijack for other sprite displays
    SetSpriteInvisibility(SPRITE_ARR_ID_SMALLICON, TRUE);
    SetSpriteInvisibility(SPRITE_ARR_ID_EXTRATYPES, TRUE);
    SetSpriteInvisibility(SPRITE_ARR_ID_EXTRATYPES + 1, TRUE);
    PrintMonInfo();
}
...
static void TilemapFiveMovesDisplay(u16 *dst, u16 palette, bool8 remove)
{
    ...
    if (!remove)
    {
        for (i = 0; i < 15; i++)
        {
            ...
            dst[id + i + 0x40] = gSummaryScreenWindow_Tilemap[i + 15] + palette;
            dst[id + i + 0x60] = gSummaryScreenWindow_Tilemap[i + 30] + palette;
        }
    }
    else // Remove
    {
        for (i = 0; i < 15; i++)
        {
            dst[id + i] = gSummaryScreenWindow_Tilemap[i + 45] + palette;
            dst[id + i + 0x20] = gSummaryScreenWindow_Tilemap[i + 60] + palette;
            dst[id + i + 0x40] = gSummaryScreenWindow_Tilemap[i + 60] + palette;
            dst[id + i + 0x60] = gSummaryScreenWindow_Tilemap[i + 60] + palette;
        }
    }
}
...
static void PrintNotEggInfo(void)
{
    ...
    StringAppend(gStringVar1, gStringVar2);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gStringVar1, 2, 17, 0, 1);
    GetMonNickname(mon, gStringVar1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gStringVar1, 16, 1, 0, 1);
    ...
    PrintHeldItemName();
    StringCopy(gStringVar1, gText_Item);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_DEX_NUMBER, gStringVar1, 2, 1, 0, 1);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_DEX_NUMBER);
...
static void PrintGenderSymbol(struct Pokemon *mon, u16 species)
{
    if (species != SPECIES_NIDORAN_M && species != SPECIES_NIDORAN_F)
    {
        ...
        {
        case MON_MALE:
            PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gText_MaleSymbol, 89, 1, 0, 3);
            break;
        case MON_FEMALE:
            PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gText_FemaleSymbol, 89, 1, 0, 4);
            break;
        }
    }
}
...
static void PrintMonDexNo(void)
{
    ...
    windowId = AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_POKEDEX_NO);
    PrintTextOnWindow(windowId, gText_EmptyString8, 0, 1, 0, 1);
...
static void PrintMonOTName(void)
{
    ...
    if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE)
    {
        ...
        x = GetStringCenterAlignXOffset(1, sMonSummaryScreen->summary.OTName, 56) + 72;
...
static void PrintMonOTID(void)
{
    ...
    if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE)
    {
        ...
        PrintTextOnWindow(windowId, gText_IDNumber, 0, 1, 0, 1);
...
static void PrintHeldItemName(void)
{
    ...
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_SPECIES, text, 2, 0, 0, 0);
}
...
static void BufferRightColumnStats(void)
{
    ...
    DynamicPlaceholderTextUtil_Reset();
	BufferStat(gStringVar1, natureMod[STAT_SPATK - 1], sMonSummaryScreen->summary.spatk, 0, 7);
	BufferStat(gStringVar2, natureMod[STAT_SPDEF - 1], sMonSummaryScreen->summary.spdef, 1, 7);
	BufferStat(gStringVar3, natureMod[STAT_SPEED - 1], sMonSummaryScreen->summary.speed, 2, 7);
...
static void PrintExpPointsNextLevel(void)
{
    ...
    PrintTextOnWindow(windowId, gText_ExpPoints, 0, 1, 0, 1);
    PrintTextOnWindow(windowId, gText_NextLv, 0, 17, 0, 1);

    ConvertIntToDecimalStringN(gStringVar1, sum->exp, STR_CONV_MODE_RIGHT_ALIGN, 7);
    x = GetStringRightAlignXOffset(1, gStringVar1, 42) + 80;
...
static void PrintMoveDetails(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            PrintTextOnWindowSmall(windowId, gMoveDescriptionPointers[move - 1], 6, 1, 0, 0);
        }
        else
        {
            PrintTextOnWindow(windowId, gContestEffectDescriptionPointers[gContestMoves[move].effect], 6, 1, 0, 0);
...
static void CreateMoveExtraTypeIcons(void)
{
    ...
    for (i = SPRITE_ARR_ID_EXTRATYPES; i < SPRITE_ARR_ID_EXTRATYPES + 2; i++)
    {
        if (sMonSummaryScreen->spriteIds[i] == SPRITE_NONE)
            sMonSummaryScreen->spriteIds[i] = CreateSprite(&gSpriteTemplate_MoveTypes, 0, 0, 2);
...
static void SetMonTypeIcons(void)
{
    ...
    if (summary->isEgg)
    {
        SetTypeSpritePosAndPal(TYPE_NORMAL, 188, 48, SPRITE_ARR_ID_TYPE);
    ...
    else
    {
        ...
        {
            SetTypeSpritePosAndPal(gBaseStats[summary->species].type1, 172, 48, SPRITE_ARR_ID_TYPE);
            SetTypeSpritePosAndPal(gBaseStats[summary->species].type2, 204, 48, SPRITE_ARR_ID_TYPE + 1);
            SetSpriteInvisibility(SPRITE_ARR_ID_TYPE + 1, FALSE);
        }
        else
        {
            SetTypeSpritePosAndPal(gBaseStats[summary->species].type1, 188, 48, SPRITE_ARR_ID_TYPE);
            SetSpriteInvisibility(SPRITE_ARR_ID_TYPE + 1, TRUE);
        }
    }
}
...
{
    struct PokeSummary *summary = &sMonSummaryScreen->summary;
    SetSpriteInvisibility(SPRITE_ARR_ID_EXTRATYPES, FALSE);
    if (gBaseStats[summary->species].type1 != gBaseStats[summary->species].type2)
    {
        SetTypeSpritePosAndPal(gBaseStats[summary->species].type1, 32, 20, SPRITE_ARR_ID_EXTRATYPES);
        SetTypeSpritePosAndPal(gBaseStats[summary->species].type2, 64, 20, SPRITE_ARR_ID_EXTRATYPES + 1);
        SetSpriteInvisibility(SPRITE_ARR_ID_EXTRATYPES + 1, FALSE);
    }
    else
    {
        SetTypeSpritePosAndPal(gBaseStats[summary->species].type1, 48, 20, SPRITE_ARR_ID_EXTRATYPES);
        SetSpriteInvisibility(SPRITE_ARR_ID_EXTRATYPES + 1, TRUE);
    }
    SetSpriteInvisibility(SPRITE_ARR_ID_SMALLICON, FALSE);
}
...
{
    struct PokeSummary *summary = &sMonSummaryScreen->summary;
    //x and y are +32 +32 here (sprite origin is centered)
    u8 spriteId = CreateSprite(&gMultiuseSpriteTemplate, 48, 88, 5);
...
static void CreateMonMarkingsSprite(struct Pokemon *mon)
{
    ...
    if (sprite != NULL)
    {
        ...
        sMonSummaryScreen->markingsSprite->x = 80;
        sMonSummaryScreen->markingsSprite->y = 44;
...
static void CreateSetStatusSprite(void)
{
    ...
    if (*spriteId == SPRITE_NONE)
        *spriteId = CreateSprite(&sSpriteTemplate_StatusCondition, 80, 36, 0);
...
static void CreateMoveSelectorSprites(u8 idArrayStart)
{
    ...
    if (sMonSummaryScreen->currPageIndex >= PSS_PAGE_BATTLE_MOVES)
    {
        ...
        {
            spriteIds[i] = CreateSprite(&sMoveSelectorSpriteTemplate, i * 16 + 112, 32, subpriority);
            if (i == 0)
